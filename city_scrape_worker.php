...
// House-key pool: load APIFY_KEY_1..21 from .env and seed the apify_keys table
// (which tracks per-key quota/cooldown state). The worker rotates through these
// keys via cs_pick_apify_key(); on 402/403/429 a key is cooled down and the ZIP
// is requeued. $tokensByRef maps key_ref (APIFY_KEY_n) -> the actual token.
$tokensByRef = [];
for ($i = 1; $i <= 21; $i++) {
    $ref = "APIFY_KEY_$i";
...
if (!empty($tokensByRef)) {
    ...
    csw_log('WARNING: no APIFY_KEY_n found in .env — worker cannot scrape');
}
...
while ((time() - $startedAt) < CSW_TIME_BUDGET) {
    ...
    // Load the parent job for keyword/country.
    ...
    csw_log("scraping job=$jobId zip=$zipStr keyword=$keyword");

    $result = csw_scrape_zip($keyword, $zipStr, $country, $tokensByRef, $supabaseCache, $zipId, $workerId);
 ...
 */
function csw_scrape_zip($keyword, $zip, $country, $tokensByRef, $cache, $zipId, $workerId)
{
    // House-key rotation: pick an available key from the pool. If none is available
    // (all cooled down / quota exhausted), requeue the ZIP and let cron retry later.
    $keyRef = cs_pick_apify_key();
    if (!$keyRef || empty($tokensByRef[$keyRef])) {
        return ['outcome' => 'requeue', 'reason' => 'no available house key'];
    }
    $token = $tokensByRef[$keyRef];
    ...
    if ($start['code'] === 402 || $start['code'] === 403 || $start['code'] === 429) {
        // Key exhausted/rate-limited — cool it down and requeue for the next key.
        cs_cooldown_apify_key($keyRef);
        return ['outcome' => 'requeue', 'reason' => "key $keyRef quota/start http {$start['code']}"];
    }
